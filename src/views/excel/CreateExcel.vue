...
<v-container fill-height fluid grid-list-xl>
  <v-layout justify-center wrap>
    <v-flex xs12 md12>
      <material-card color="green" title="ارسال فایل اکسل محصولات" text="فایل اکسل ضروری است">
        <v-form>
          <v-container py-0>
            <v-layout wrap>
              <v-flex xs12 md6 text-xs-right>
                <v-file-input accept=".xlsx, .xls" v-model="excel" label="فایل "></v-file-input>
              </v-flex>
              <v-flex xs12 text-xs-left>
                <v-btn :loading="loading" class="mx-0 font-weight-light" color="success" :disabled="!isExcelValid" @click="SendExcel()">
                  ارسال
                </v-btn>
              </v-flex>
            </v-layout>
...
export default {
  ...
  methods: {
    ...
    SendExcel() {
      ...
      this.$store.dispatch('sendExcelRequest', formData)
        .then(response => {
          this[l] = false
          this.loader = null
          this.snack_success()
